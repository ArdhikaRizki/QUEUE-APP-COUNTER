...
export const apiClaimQueue = async () => {
  try {
    console.log("🎫 Claiming queue...", {
      url: `${API_BASE_PATH}/claim`,
      fullUrl: `${process.env.NEXT_PUBLIC_API_URL}${API_BASE_PATH}/claim`
    });
    ...
    console.log("✅ Claim queue response:", res.data);
    return res.data;
  } catch (e) {
    console.error("❌ Claim queue error:", e);
